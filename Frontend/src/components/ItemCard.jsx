export default function ItemCard({ item, onClaimClick }) {
  return (
    <div className="flex flex-col sm:flex-row items-start justify-between p-4 bg-white/60 border rounded-lg shadow-sm hover:shadow-md transition">
      <div className="flex gap-4 w-full">
        <div className="w-20 h-20 rounded-lg bg-accent-light flex items-center justify-center text-accent-dark font-semibold text-2xl flex-shrink-0">{item.title?.charAt(0) || '?'}</div>
        <div className="flex-1">
          <div className="flex items-center gap-3">
            <h3 className="text-lg font-medium">{item.title}</h3>
            <span className={`text-xs px-2 py-1 rounded-full ${item.found ? 'bg-accent-light text-accent-contrast' : 'bg-slate-100 text-slate-600'}`}>{item.found ? 'Ditemukan' : 'Hilang'}</span>
            {item.claimed && <span className="ml-2 text-xs px-2 py-1 rounded bg-emerald-100 text-emerald-700">Sudah Diklaim</span>}
          </div>
