    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <label className="flex flex-col">
          ...
          <select value={found} onChange={e => setFound(e.target.value)} className="mt-2 p-3 border rounded-lg shadow-sm focus:ring-4 focus:ring-accent/30">
        ...
        <label className="flex flex-col md:col-span-2">
          ...
          <input value={title} onChange={e => setTitle(e.target.value)} className="mt-2 p-3 border rounded-lg shadow-sm focus:ring-4 focus:ring-accent/30" placeholder="Contoh: Dompet hitam" />
        </label>
      </div>

      <label className="flex flex-col">
        <span className="text-sm text-slate-600 font-medium">Deskripsi</span>
        <textarea value={description} onChange={e => setDescription(e.target.value)} className="mt-2 p-3 border rounded-lg shadow-sm focus:ring-4 focus:ring-accent/30" rows={5} placeholder="Detail barang, ciri-ciri, dll." />
      </label>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <label className="flex flex-col">
          <span className="text-sm text-slate-600 font-medium">Lokasi terakhir / ditemukan</span>
          <input value={location} onChange={e => setLocation(e.target.value)} className="mt-2 p-3 border rounded-lg shadow-sm focus:ring-4 focus:ring-accent/30" placeholder="Contoh: Perpustakaan, Lantai 2" />
        </label>

        <label className="flex flex-col">
          <span className="text-sm text-slate-600 font-medium">Kontak (opsional)</span>
          <input value={contact} onChange={e => setContact(e.target.value)} className="mt-2 p-3 border rounded-lg shadow-sm focus:ring-4 focus:ring-accent/30" placeholder="Email atau nomor HP" />
        </label>
      </div>
